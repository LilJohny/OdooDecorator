...
#  - method._api: msg_decorator function, used for re-applying msg_decorator
#
...
def onchange(*args):
    """Return a msg_decorator to decorate an onchange method for given fields.
...
def depends(*args):
    """ Return a msg_decorator that specifies the field dependencies of a "compute"
...
def depends_context(*args):
    """ Return a msg_decorator that specifies the context dependencies of a
...
def returns(model, downgrade=None, upgrade=None):
    """ Return a msg_decorator for methods that return instances of ``model``.
        ...
        The msg_decorator adapts the method output to the api style: ``id``, ``ids`` or
